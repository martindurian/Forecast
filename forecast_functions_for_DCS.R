...
prepare_df_fun <- function(Data_struct_fun) map(Data_struct_fun$Out_DF_list_outdoors, function (outdoor_df){
  ...
  outdoor_inc_tec_df <- outdoor_df %>%
    ...
    select(TimeStampPosixct, consumption = Watt.hour.amount.Corrected., ambient = Outside.air.temp., OpeMode, Operation.control.mode, UniqueIdentifier, UnitType, Line_Name, SystemIdentifier, ope_time_out = Outdoor.unit.operation.time, matches('^Te.mean.value|^Tc.mean.value|^Tes.mean.value|^Tcs.mean.value')) %>%   #'^T.*\\.mean\\.value' %>% 
  ...
  indoors_average_values <- Data_struct_fun %>% 
    ...
    select(TimeStampPosixct, Avg..setting.temp., Indoor.unit.thermostat.ON.time.accumulation, Indoor.unit.operation.time.accumulation, Suction.air.temp., UnitType, SystemIdentifier, UniqueIdentifier) %>% 
    ...
    mutate(unit_size = get_unit_size(UnitType),
           setpoint_weight = if_else(Indoor.unit.operation.time.accumulation == 0, 0, unit_size * Indoor.unit.thermostat.ON.time.accumulation/Indoor.unit.operation.time.accumulation),
           setpoint_factor = Avg..setting.temp.* setpoint_weight, 
           ope_time_factor = Indoor.unit.operation.time.accumulation/60 * unit_size,
           suction_factor = Suction.air.temp. * setpoint_weight
           # suction_factor_opetime = Suction.air.temp. * unit_size * Indoor.unit.operation.time.accumulation/60
           )
  
  indoors_average_values <- indoors_average_values %>% 
    group_by(TimeStampPosixct) %>% 
    summarise(unit_size_sum = sum(unit_size, na.rm = TRUE),
              setpoint_factor_sum = sum(setpoint_factor, na.rm = TRUE), 
              setpoint_weight_sum = sum(setpoint_weight, na.rm = TRUE), 
              average_setpoint=setpoint_factor_sum/setpoint_weight_sum,
              ...
              average_suction = suction_factor_sum/setpoint_weight_sum
    ...
    mutate(average_suction = replace_na(average_suction, 0),
           ...
           avg_suction_lag_diff = average_suction - lag(average_suction)) %>%
    select(-contains("sum"))
  ...
  total_inc_tec_tsbl <- outdoor_inc_tec_df %>% 
    ...
    mutate(hd = hour(TimeStampPosixct),
           my = month(TimeStampPosixct),
           wd = wday(TimeStampPosixct, week_start = getOption("lubridate.week.start", 1)),
           tow = hour(TimeStampPosixct) + lubridate::minute(TimeStampPosixct)/60 + wday(TimeStampPosixct, week_start = getOption("lubridate.week.start", 1)) * 24,
           OpeMode = factor(OpeMode, levels = c("Cooling", "Heating", "HeRec", "Both", "Idle"))) %>% 
...
corr_coef <- function(df) as_tibble(df) %>% 
  ...
  corrr::correlate() %>% 
  filter(term == "consumption") %>% 
  select_if(~any(!is.na(.)))

corr_network <- function(df) as_tibble(df) %>% 
  select_if(is.numeric) %>% 
  corrr::correlate() %>%
...
apply_eco_shut_down_cooling <- function(df){
  ...
  setback_df <- mutate(df,
                       dummy.worktime_out = if_else(OpeMode == "Cooling" & non_working_time, 0, dummy.worktime_out),
                       ope_time_out = if_else(OpeMode == "Cooling" & non_working_time, 0, ope_time_out),
                       average_ope_time = if_else(OpeMode == "Cooling" & non_working_time, 0, average_ope_time)
                       )
  return(setback_df)
} 

apply_eco_non_work_time_setback_heating <- function(df, Tstbk_heat = 18, continuous_run = TRUE) {
  if (!("non_working_time" %in% names(df))) df <- add_non_working_time(df)
  setback_df <- mutate(df,
  ...
  setback_df <- mutate(setback_df, avg_suction_lag_diff = average_suction - lag(average_suction))
  if(continuous_run) setback_df <- mutate(setback_df, dummy.worktime = 1,
                                          average_ope_time = 1)
  return(setback_df)
}
...
compare_cons_fun <- function (training_df, forecast_real, forecast_stbck){
  ...
  comparison_df <- as_tibble(forecast_real) %>%
    group_by(.model) %>% 
    summarise(fcst_sum = sum(.mean, na.rm = T)) %>% 
    full_join(forecast_stbck %>% 
                as_tibble() %>% 
                group_by(.model) %>% 
                summarise(fcst_setback_sum = sum(.mean, na.rm = T)) ,
              by = ".model") %>% 
...
add_mode_columns <- function (df, ope_mode, model_formula){
  # browser()
  orig_names <- names(df)
  df <- mutate(df, data = map(hourly_df, ~filter(.x, OpeMode == ope_mode)))
  df <- mutate(df, corr_coef = map(data, possibly(~corr_coef(.x), otherwise = NA_real_)))
  df <- mutate(df, corr_network = map(data, possibly(~corr_network(.x), otherwise = NA_real_)))
...
facet_eco_lbl <- function(lbl) case_when(lbl == "forecast_real" ~ "without ECO", 
                                         lbl == "forecast_stbck" ~ "with ECO")
...
weekly_plt_comparison_fun <- function (forecast_real, forecast_stbck, consumption_var) {
  ...
  df <- imap_dfr(df_lst, function (df, df_name) {
    ...
    if (!("Week_day_consumption" %in% names(df))) df <- as_tibble(df) %>% 
        ...
        filter(!(Week_Day == 0 & is.na(Week_Day_Name)))
    return(df)
  })
  # main plt
 plt <- weekly_plt_fun(df)
 plt <- plt +
    facet_wrap(~ df_name, ncol = 2, labeller = as_labeller(facet_eco_lbl))
...
setpoint_plt_comparison_fun <- function (forecast_real, forecast_stbck, consumption_var, histogram_var) {
  ...
  df <- imap(df_lst, function (df, df_name) as_tibble(df) %>% 
               ...
               mutate(df_name = df_name)) %>% 
    bind_rows()
  ope_mode <- pull(filter(distinct(df, OpeMode), !is.na(OpeMode)), OpeMode)
...
flextable_pretty <- function(tbl){
  format_fun <- function (x) if_else(abs(x) < 1, sprintf("%#.3g", x), prettyNum(x, digits = 3))
  num_names <-  names(select(as_tibble(tbl$body$dataset), where(is.numeric)))
  format_lst <- setNames(rep(list(format_fun), length(num_names)), num_names)
  tbl <- border_remove(tbl) %>% 
    set_formatter(values = format_lst) %>% 
    border(border.bottom = fp_border(color = daikin_cols("dark grey"), width = 1), part = "header") %>%
    border(border.bottom = fp_border(color = daikin_cols("grey 60"), width = 0.3), part = "body")
  return(tbl)
}
